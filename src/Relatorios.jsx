...
function periodoUltimos3Meses() {
  const hoje = new Date();
  return { di: toHtml(new Date(hoje.getFullYear(), hoje.getMonth() - 2, 1)), df: toHtml(hoje) };
}
...
const Icons = {
  ...
  Spinner:  () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5 animate-spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>,
};
...
export default function Relatorios({ onNavigate }) {
  ...
  const [pdf, setPdf]           = useState(null); // { url, nome }
  ...
  // Gerar PDF — abre no popup
  async function visualizar(tipo) {
    if (!periodo.di || !periodo.df) {
      setToast({ ok: false, text: "Selecione o período antes de gerar o relatório." }); return;
    }
    if (escopo === "especifico" && !funcSel) {
      setToast({ ok: false, text: "Selecione um funcionário." }); return;
    }
    ...
    <div className="flex h-screen bg-slate-900 text-slate-100 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-slate-800 border-b border-slate-700 px-8 py-4 flex-shrink-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Gere e visualize espelhos e relatórios de horas em PDF</p>
        </header>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 flex flex-col gap-6">
            ...
            <div>
              ...
              <div className="flex gap-3 mb-4">
                {[{ id: "todos", label: "Todos" }, { id: "especifico", label: "Específico" }].map(({ id, label }) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 flex flex-col gap-5">
              ...
              <div className="border-t border-slate-700 pt-4 flex items-center justify-between">
                <span className="text-xs text-slate-500">Formato: <span className="text-slate-300 font-mono">PDF</span></span>
                <button onClick={() => visualizar("mte")} disabled={loading.mte}
                  ...
                  {loading.mte ? <><Icons.Spinner/> Gerando...</> : <><Icons.Eye/> Visualizar</>}
                </button>
              </div>
            </div>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 flex flex-col gap-5">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    Resumo de horas trabalhadas, extras, faltas e saldo do período.
                  </p>
                </div>
              </div>
              <div className="border-t border-slate-700 pt-4 flex items-center justify-between">
                <span className="text-xs text-slate-500">Formato: <span className="text-slate-300 font-mono">PDF</span></span>
                <button onClick={() => visualizar("horas")} disabled={loading.horas}
                  ...
                  {loading.horas ? <><Icons.Spinner/> Gerando...</> : <><Icons.Eye/> Visualizar</>}
                </button>
              </div>
            </div>
          </div>
